...
def vis_flow_batch(flow_batch):
	bs = flow_batch.shape[0]
	eps = sys.float_info.epsilon
	UNKNOWN_FLOW_THRESH = 1e9
	UNKNOWN_FLOW = 1e10

	rgb_out = np.zeros([flow_batch.shape[0],flow_batch.shape[1],flow_batch.shape[2],3]).astype(int)
...
# def vis_flow_norm(flow, max_u=None, min_u=None, max_v=None, min_v=None):
def vis_flow_norm(flow, max_rad=None):
	eps = sys.float_info.epsilon
	UNKNOWN_FLOW_THRESH = 1e9
	UNKNOWN_FLOW = 1e10
